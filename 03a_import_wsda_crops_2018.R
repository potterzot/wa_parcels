...
wsda_gdb <- "/usb/big/data/cropland/2018WSDACropDistribution.gdb"
wsda_features <- ogrListLayers(dsn = wsda_gdb)

# Shapes
feature <- "WSDACrop_2018"
...
# Add an object ID
dbExecute(con, "ALTER TABLE wsdacrop_2018 ADD COLUMN oid SERIAL UNIQUE;")

# Create indexes for the shapes and centroids
dbExecute(con, "CREATE INDEX wsdacrop_2018_centroid_idx ON wsdacrop_2018 USING GIST (centroid);")
dbExecute(con, 'CREATE INDEX wsdacrop_2018_shape_idx ON wsdacrop_2018 USING GIST (shape);')

# Analyze for improved query speed
dbExecute(con, "VACUUM ANALYZE wsdacrop_2018;")
